...
if (isset($_POST['supprimer_tous_scrutins'])) {
    ...
    $pdo->exec("TRUNCATE TABLE codes_utilises");
...
if (isset($_POST['afficher_codes'])) {
    ...
    // NOUVELLE VERSION : join avec codes_utilises
    $stmt_codes = $pdo->query("
        SELECT c.id, c.code,
               CASE WHEN cu.code_id IS NULL THEN 0 ELSE 1 END AS utilise
        FROM codes c
        LEFT JOIN codes_utilises cu ON cu.code_id = c.id
        GROUP BY c.id
        ORDER BY c.id ASC
    ");
...
if (isset($_POST['supprimer_code'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM codes_utilises WHERE code_id = ?");
...
if (isset($_POST['supprimer_codes_utilises'])) {
    $stmt = $pdo->query("SELECT DISTINCT code_id FROM codes_utilises");
    $codes = $stmt->fetchAll(PDO::FETCH_COLUMN);

    if (!empty($codes)) {
        $in = implode(',', array_fill(0, count($codes), '?'));

        $stmt = $pdo->prepare("DELETE FROM codes_utilises WHERE code_id IN ($in)");
        $stmt->execute($codes);

        $stmt = $pdo->prepare("DELETE FROM codes WHERE id IN ($in)");
        $stmt->execute($codes);
    }
...
// Liste des scrutins
$scrutins = $pdo->query("
    SELECT s.id, s.nom, s.date_creation, COUNT(v.id) AS nb_votes
    FROM scrutins s
...
<table>
<tr><th>ID</th><th>Nom</th><th>Date (Votes)</th><th>Actions</th></tr>
...
<button type='submit' name='afficher_codes'>Afficher</button>
<button type='submit' name='supprimer_tous_codes' onclick="return confirm('Supprimer tous les codes ?')">Tout supprimer</button>
<button type='submit' name='supprimer_codes_utilises' onclick="return confirm('Supprimer tous les codes utilisés ?')">Supprimer utilisés</button>
</form>
